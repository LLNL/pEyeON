...
class testChecksum(unittest.TestCase):
    ...
    def test_hash_file_sha256(self):
        result = checksum.Checksum(self.temp_file.name, algorithm='sha256', expected_checksum=self.expected_sha256)
        self.assertEqual(result, self.expected_sha256)
    
    def test_hash_file_md5(self):
        result = checksum.Checksum(self.temp_file.name, algorithm='md5', expected_checksum=self.expected_md5)
        self.assertEqual(result, self.expected_md5)
    
    def test_hash_file_sha1(self):
        result = checksum.Checksum(self.temp_file.name, algorithm='sha1', expected_checksum=self.expected_sha1)
        self.assertEqual(result, self.expected_sha1)


    #Check fails
    def test_hash_file_sha256_fail(self):
        result = checksum.Checksum(self.temp_file.name, algorithm='sha256', expected_checksum="fj93u2j9ji")
        self.assertNotEqual(result, "fj93u2j9ji")

    def test_hash_file_md5_fail(self):
        result = checksum.Checksum(self.temp_file.name, algorithm='md5', expected_checksum="324eqwr2")
        self.assertNotEqual(result, "324eqwr2")

    def test_hash_file_sha1_fail(self):
        result = checksum.Checksum(self.temp_file.name, algorithm='sha1', expected_checksum="32qreq42")
